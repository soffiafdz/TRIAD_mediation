...
# Un-Z
cog.hcv.dt  <- DTunscale(DT_lc[!HC %like% "HVR"], scalevars = "Yp",
                         origs = "COG", replace = F, visit.col = "I")
cog.hvr.dt  <- DTunscale(DT_lc[HC %like% "HVR"], scalevars = "Yp",
                         origs = "COG", replace = F, visit.col = "I")
...
### HC vs HVR
f_plot1  <- sprintf("plots/%s.%s", "adni_trajectory_hcv-hvr_mlm",
                    c("png", "tiff")) |> here()
if(!file.exists(f_plot1[[1]])
...
### Factor renaming
  ...
  pdx <- ggplot(DTp1, aes(x = AGE, y = Y, colour = D, lty = HC)) +
    ...
    geom_line(aes(y = Yp), stat = "smooth",  method = "lm",
              formula = y ~ poly(x, 2), linewidth = .3, alpha = .7) +
    scale_colour_manual(values = cbPalette[c(4,2,7)]) +
    #scale_fill_manual(values = cbPalette[c(4,2,7)]) +
...
## Racket's EDT
f_plot2  <- sprintf("plots/%s.%s", "adni_trajectory_edt_mlm",
                    c("png", "tiff")) |> here()
if(!file.exists(f_plot2[[1]])
   | (!file.exists(f_plot2[[2]]) & PrintTIFF)
   | ReDoPlots) {

  ### Factor renaming
  DTp2 <- DT[, .(PTID, AGE, DX)][DT_lr, on = .(PTID = ID, AGE)] |>
  setnames(c("PTID", "DX"), c("ID", "D")) |>
  melt(measure = patterns("^X"), variable.name = "BL", value.name = "X")
  DTp2["Xb", on = "BL", Yp := Ybp]
  DTp2["Xb", on = "BL", Xb := X]
  DTp2["X", on = "BL", Xl := X]
  DTp2[, Ybp := NULL]
  DTp2[, BL := factor(BL, levels = c("Xb", "X"),
                      labels = c("Baseline only", "All visits"))]
  DTp2[, D := factor(D, labels = c("CH", "MCI", "AD"))]
  DTp2[, HC := factor(HC, levels = rev(hcvars), labels = c("HVR", "Volume"))]
  DTp2[, S := factor(S, labels = c("F", "M"))]

  p_main <- ggplot(DTp2[!is.na(D)], aes(x = Y, y = X, colour = D)) +
    theme_classic(base_size = 11) +
    theme(text = element_text(size = 11),
          axis.text.y = element_text(size = 10),
          axis.text.x = element_text(size = 10)) +
    geom_line(aes(y = Xl, group = ID), linewidth = .25, alpha = .25) +
    geom_point(aes(y = Xb), size = .35, alpha = .2) +
    #geom_smooth(aes(y = Yp), method = "lm", formula = y ~ poly(x, 2),
                #linewidth = .5, alpha = .2) +
    #geom_line(aes(group = DX, y = PRED), linewidth = .5, alpha = .2) +
    scale_colour_manual(values = cbPalette[c(4,2,7)]) +
    #scale_fill_manual(values = cbPalette[c(4,2,7)]) +
    facet_grid(cols = vars(HC), rows = vars(BL)) +
    xlab("Estimated disease timeline") +
    ylab("HC: Z-score")

  psex <- ggplot(DTp2, aes(x = Yp, y = X, colour = S, lty = HC)) +
    theme_classic(base_size = 11) +
    theme(text = element_text(size = 11),
          axis.text.y = element_text(size = 10),
          axis.text.x = element_text(size = 10)) +
    ...
    scale_colour_manual(values = cbPalette[c(8,3)]) +
    #scale_fill_manual(values = cbPalette[c(8,3)]) +
    facet_wrap(vars(BL), ncol = 1, scales = "free") +
    labs(subtitle = "Sex", colour = "Sex", lty = "HC") +
    xlab("Est. disease timeline") +
    ylab("HC: Z-score")

  pa4 <- ggplot(DTp2, aes(x = Yp, y = X, colour = A4, lty = HC)) +
    theme_classic(base_size = 11) +
    theme(text = element_text(size = 11),
          #axis.title.x = element_blank(),
          axis.title.y = element_blank(),
  ...
  p2 <- p_main / (psex + pa4) +
  plot_annotation(title = "Neurodegeneration Trajectories in Disease Timeline",
                  subtitle = paste("MLM:",
                                   "Predicting EDT with HC volume & HVR;",
                                   "random intercepts and slopes"),
                  caption = paste("Estimated disease timeline (EDT)",
                                  "was calculated using Raket's method from:",
                                  "MMSE & CDRSB")) +
  plot_layout(heights = c(1,.8), guides = "collect", axes = "collect")

  if(!file.exists(f_plot2[[1]]) | ReDoPlots){
    ggsave(f_plot2[[1]], width = 7, height = 8, units = "in", dpi = 400)
  }

  if(PrintTIFF & (!file.exists(f_plot2[[2]]) | ReDoPlots)){
    ggsave(f_plot2[[2]], width = 7, height = 8, units = "in",
           device = "tiff", dpi = 400)
  }
}

### Cognition
f_plot3  <- sprintf("plots/%s.%s", "adni_trajectory_cog_mlm",
                    c("png", "tiff")) |> here()
if(!file.exists(f_plot3[[1]])
  ...
  ggplot(aes(x = AGE, y = COG, colour = D)) +
    ...
    theme(text = element_text(size = 11),
          ...
          axis.text.x = element_text(size = 10),
          legend.position = "none") +
    geom_line(aes(group = ID), linewidth = .25, alpha = .25) +
  ...
  ggplot(aes(x = AGE, y = COG)) +
    ...
    scale_colour_manual(values = cbPalette[c(4,2,7)]) +
    geom_hline(yintercept = 0, lty = "dashed", colour = cbPalette[1]) +
    geom_smooth(aes(colour = D, fill = D), method = "lm",
                formula = y ~ poly(x, 2), linewidth = .5, se = T, alpha = .2) +
    geom_smooth(colour = cbPalette[1], fill = cbPalette[1], method = "lm",
                formula = y ~ poly(x, 2), linewidth = .7, se = T, alpha = .2) +
    labs(subtitle = "Group trajectories",
         colour = "Dx", fill = "Dx") +
    ylab("Decline (observed)") +
    xlab("Age (years)")
  ...
  phc <-  DTp3["HVR", on = "HC"] |>
    ...
    geom_smooth(aes(x = Ypu), linewidth = .3, alpha = .1, method = "lm",
                formula = y ~ poly(x, 2)) +
  ...
  if(!file.exists(f_plot3[[1]]) | ReDoPlots){
    ggsave(f_plot3[[1]], width = 7, height = 8, units = "in", dpi = 400)
  }

...
if (file.exists(fpaths[["RDS"]][4])) {
  cog.dt <- fpaths[["RDS"]][4] |> readRDS()
} else {
  fpaths[["SRC"]][4] |> source()
}

## RAKET EDT
if (file.exists(fpaths[["RDS"]][5])) {
  raket.dt <- fpaths[["RDS"]][5] |> readRDS()
} else {
  fpaths[["SRC"]][5] |> source()
}
rm(fpaths)
...
DT_r <- DT[, .(SEX, APOE4, HCv, HVR), keyby = .(PTID, EXAMDATE)][raket.dt]
...
DT_r <- tic.dt[hchvr.bl.dt][DT_r][EXAMDATE.bl <= EXAMDATE] |>
  unique() |>
...
DT_r <- DTclean(
  DT_r,
  scalevars = c("HCv", "HVR", "HCv.bl", "HVR.bl"),
  centervars = "AGE"
)
...
DT_lr <- melt(
  DT_r,
  measure = patterns(VAL = "(HCv|HVR).scl", VAL.bl = "(HCv|HVR).bl.scl"),
  variable.name = "HC"
)

...
## Path plots
## HVR full models
fname       <- here("data/rds/med-mods_hvr_raket_ug.rds")
...
for (i in seq_along(mod_hvr)) {
  fname     <- here(sprintf("plots/mediation_paths_hvr-%i_ug_full.pdf", i))
  if (!file.exists(fname) & print_plots) {
    lavaanPlot2(model = mod_hvr[[i]], labels = c(SEX_n = "SEX"),
  ...
  fname     <- here(sprintf("plots/mediation_paths_hvr-%i_ug_sign.pdf", i))
  if (!file.exists(fname) & print_plots) {
...
## HCv full models
fname       <- here("data/rds/med-mods_hcv_raket_ug.rds")
...
for (i in seq_along(mod_hcv)) {
  fname     <- here(sprintf("plots/mediation_paths_hcv-%i_ug_full.pdf", i))
  if (!file.exists(fname) & print_plots) {
    lavaanPlot2(model = mod_hcv[[i]], labels = c(SEX_n = "SEX"),
                graph_options = list(rankdir = "LR"),
                node_options = list(shape = "box"),
                edge_options = list(color = "grey"),
                coef_labels = T, stand = T, stars = "regress") |>
  ...
  fname     <- here(sprintf("plots/mediation_paths_hcv-%i_ug_sign.pdf", i))
  if (!file.exists(fname) & print_plots) {
